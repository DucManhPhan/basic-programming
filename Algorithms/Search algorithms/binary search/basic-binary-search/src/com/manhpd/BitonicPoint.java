 ...
 * <p>
 * Suppose we are given an integer array nums that is guaranteed to be a bitonic one,
 * and return any `i` such that `nums[0] < nums[1] < ... < nums[i - 1] < nums[i] > nums[i + 1] > ... > nums[nums.length - 1]`.
 * <p>
 * Constraints:
 * - `3 ≤ nums.length ≤ 10^4`
 * - `0 ≤ nums[i] ≤ 10^6`
...
//        int[] nums = {0, 10, 5, 2};
    ...
    private static int findBitonicPoint(int[] nums) {
        ...
        while (left + 1 < right) {
            ...
            if (mid > 0 && mid < nums.length && nums[mid] > nums[mid - 1] &&
                    nums[mid] > nums[mid + 1]) {
